...
| `modules/0-preflight.sh` | Environment validation: OS, user, arch, graphics, bootstrap packages |
...
| `lib/common.sh` | Shared logging helpers, `s()` sudo wrapper, file-install utilities |
...
- **Modules load in numeric order** (`0-preflight`, `1-system`, `2-workstation`, `3-desktop`, `4-optional`). Preflight runs first, system-level ops before later modules can rely on installed packages.
