...
section "launch-or-focus (TUI/webapp/base), webapp-install, managed symlinks, commands install"
...
test_tui_launches_once_then_focuses() {
    ...
    LAUNCH_MARKER="$marker" LAUNCH_APP_ID=local.tui.lazydocker "$ROOT_DIR/bin/tui-launch-or-focus" lazydocker "\$(touch $hostile)"
    LAUNCH_MARKER="$marker" LAUNCH_APP_ID=local.tui.lazydocker "$ROOT_DIR/bin/tui-launch-or-focus" lazydocker --debug
...
test_tui_concurrent_launch_deduplicates() {
    ...
    LAUNCH_MARKER="$marker" LAUNCH_APP_ID=local.tui.lazydocker SETSID_DELAY=0.3 "$ROOT_DIR/bin/tui-launch-or-focus" lazydocker &
    first_pid=$!
    sleep 0.05
    LAUNCH_MARKER="$marker" LAUNCH_APP_ID=local.tui.lazydocker SETSID_DELAY=0.3 "$ROOT_DIR/bin/tui-launch-or-focus" lazydocker &
    second_pid=$!
...
test_tui_failure_when_app_never_appears() {
    ...
    printf '[]\n' >"$WINDOWS_FILE"; : >"$LAUNCH_LOG"
    if "$ROOT_DIR/bin/tui-launch-or-focus" lazydocker 2>/dev/null; then return 1; fi
...
test_tui_different_app_gets_own_id() {
    ...
    printf '[]\n' >"$WINDOWS_FILE"; : >"$LAUNCH_LOG"
    LAUNCH_MARKER="$marker" LAUNCH_APP_ID=local.tui.btop "$ROOT_DIR/bin/tui-launch-or-focus" btop
...
test_webapp_focuses_by_cached_window_id() {
    ...
    : >"$FOCUS_LOG"
    "$ROOT_DIR/bin/webapp-launch-or-focus" notion https://www.notion.so
...
test_webapp_launches_and_caches_window_id() {
    ...
    printf '[]\n' >"$WINDOWS_FILE"; : >"$LAUNCH_LOG"
    LAUNCH_MARKER="$marker" LAUNCH_APP_ID=niri-webapp-notion "$ROOT_DIR/bin/webapp-launch-or-focus" notion https://www.notion.so
...
test_webapp_launch_by_name_only() {
    ...
    printf '[]\n' >"$WINDOWS_FILE"; : >"$LAUNCH_LOG"
    LAUNCH_MARKER="$marker" "$ROOT_DIR/bin/webapp-launch-or-focus" reddit https://www.reddit.com
...
test_webapp_concurrent_launch_deduplicates() {
    ...
    printf '[]\n' >"$WINDOWS_FILE"; : >"$LAUNCH_LOG"
    LAUNCH_MARKER="$marker" LAUNCH_APP_ID=niri-webapp-chatgpt SETSID_DELAY=0.3 "$ROOT_DIR/bin/webapp-launch-or-focus" chatgpt https://chatgpt.com &
    first_pid=$!
    sleep 0.05
    LAUNCH_MARKER="$marker" LAUNCH_APP_ID=niri-webapp-chatgpt SETSID_DELAY=0.3 "$ROOT_DIR/bin/webapp-launch-or-focus" chatgpt https://chatgpt.com &
    second_pid=$!
...
test_webapp_failure_when_app_never_appears() {
    ...
    printf '[]\n' >"$WINDOWS_FILE"; : >"$LAUNCH_LOG"
    if "$ROOT_DIR/bin/webapp-launch-or-focus" gmail https://mail.google.com 2>/dev/null; then return 1; fi
...
test_launch_or_focus_has_no_eval() {
    if grep -Fq eval "$ROOT_DIR/bin/webapp-launch-or-focus" || \
       grep -Fq eval "$ROOT_DIR/bin/tui-launch-or-focus"; then return 1; fi
}
